...
class SparseLinear(nn.Module):
    ...
    def __init__(
        ...
        if not small_world: # 1
            if connectivity is None: # 1
                self.sparsity = sparsity
                nnz = round((1.0 - sparsity) * in_features * out_features) 
                # The total number of connections in a fully connected layer is in_features * out_features.
                if in_features * out_features <= 10 ** 8: # 1
                    ...
                    # The row and column indices of non-zero elements in a sparse matrix.
                else:
                    warnings.warn(
    ...
    def forward(self, inputs):   # inputs: [bs x nvars x target_window]
